...
#define pgm_read_byte(x) *(x)
	...
	void oled_write(uint8_t dc, uint8_t data) {
		...
		if(dc > 0) { H(SS1306_PORT, SS1306_OLED_DC); }
		else { L(SS1306_PORT, SS1306_OLED_DC); }

		L(SS1306_PORT, SS1306_OLED_CS);
		for(int8_t bit = 7; bit >= 0; bit--)
		{
			L(SS1306_PORT, SS1306_OLED_CLK);
			if((1 << bit) & data) { H(SS1306_PORT, SS1306_OLED_DAT); } 
			else { L(SS1306_PORT, SS1306_OLED_DAT); }
			H(SS1306_PORT, SS1306_OLED_CLK);
		}
		L(SS1306_PORT, SS1306_OLED_CLK);
		H(SS1306_PORT, SS1306_OLED_CS);
	}
...
void oled_init() {
	L(SS1306_PORT, SS1306_OLED_RST);
	isix_wait_ms(10);
	H(SS1306_PORT, SS1306_OLED_RST);
	isix_wait_ms(10);
	...
	oled_write(0, 0xFF); // 128 is midpoint // Brightness
...
void oled_fill(uint8_t row, uint8_t col, uint8_t count, int16_t max, uint32_t pattern, int8_t pshift) {
	oled_move(row, col);
	uint8_t pstate = 0;
	for(; max > 0; max--) {
		if(count > max) {
