...
static atomic_bool ethif_ev_tx = ATOMIC_VAR_INIT(0);
static atomic_bool ethif_ev_rx = ATOMIC_VAR_INIT(0);
static atomic_bool ethif_ev_mac = ATOMIC_VAR_INIT(0);
...
{
  atomic_store(&ethif_ev_mac, true);
	isix_sem_signal_isr( netif_sem );
  LL_EXTI_ClearFlag_0_31( PHY_INT_EXTI_LINENUM(PHY_INT_EXTI_NUM));
}
...
static void  __attribute__((__noreturn__)) netif_task( void *ifc )
{
	...
	for(;;)
	{
		...
		{
#if PHY_INT_USE_INTERRUPT
			if( atomic_load(&ethif_ev_mac) )
			{
				check_link_status( ETH_DRV_PHY_ADDR, netif );
			}
#endif
			//If TX int
			if( atomic_load(&ethif_ev_tx) )
			{
				free_tx_pbufs();
			}
			//If RX int
			if( atomic_load(&ethif_ev_rx) )
      ...
      atomic_store( &ethif_ev_mac, false);
      atomic_store( &ethif_ev_tx, false);
      atomic_store( &ethif_ev_rx, false);
		}
#if !PHY_INT_USE_INTERRUPT
...
static int low_level_init(struct netif *netif)
{
  ...
  isix_request_irq(ETH_IRQn);
#if PHY_INT_USE_INTERRUPT
  LL_SYSCFG_SetEXTISource( PHY_INT_EXTI_PORT_SOURCE(PHY_INT_GPIO_PORT), PHY_INT_EXTI_PIN_SOURCE(PHY_INT_EXTI_NUM));
  LL_EXTI_EnableFallingTrig_0_31(PHY_INT_EXTI_LINENUM(PHY_INT_EXTI_NUM));
  LL_EXTI_EnableIT_0_31(PHY_INT_EXTI_LINENUM(PHY_INT_EXTI_NUM));
  LL_EXTI_ClearFlag_0_31( PHY_INT_EXTI_LINENUM(PHY_INT_EXTI_NUM));
  isix_request_irq(PHY_INT_EXTI_LINE_IRQ_N);
...
struct netif* stm32_emac_netif_create( const uint8_t *hw_addr )
{
	...
	//Set NVIC priority
  isix_irq_prio_t p1 = {ETH_DRV_IRQ_PRIO, ETH_DRV_IRQ_SUBPRIO};
	isix_set_irq_priority( ETH_IRQn , p1);
  isix_irq_prio_t p2 = {ETH_DRV_IRQ_PRIO, ETH_DRV_IRQ_SUBPRIO};
	isix_set_irq_priority( PHY_INT_EXTI_LINE_IRQ_N, p2); 
	return nifc;
}
//
...
int stm32_emac_netif_shutdown( struct netif *netif )
{
	...
	/* Enable interrupt in NVIC */
	isix_free_irq( ETH_IRQn );
#if PHY_INT_USE_INTERRUPT
	 //Disable only phy line NVIC IRQ can be shared with other pins
   LL_EXTI_DisableIT_0_31(PHY_INT_EXTI_LINENUM(PHY_INT_EXTI_NUM));
#endif
	 int ret_code = ERR_OK;
	 if( netif_task_id )
	 {
